export const QUIZ = [
  { id:"experience", q:"How long have you been using cannabis?", opts:["First time","< 1 year","1–3 years","3–5 years","5+ years"] },
  { id:"frequency",  q:"How often do you consume?",              opts:["Rarely","1–2x/week","3–4x/week","Daily","Multiple daily"] },
  { id:"usageType",  q:"Medical, recreational, or both?",        opts:["Medical (prescribed)","Recreational","Both"] },
  { id:"goal",       q:"What's your main reason for using?",     opts:["Relaxation","Pain/medical","Sleep","Creativity","Social","Exploration"] },
  { id:"method",     q:"Preferred method?",                      opts:["Joints","Vaporiser","Bong/pipe","Edibles","Oil/Tincture","Mixed"] },
  { id:"pastStrains",q:"Any strains or products you've tried?",  type:"text", placeholder:"e.g. Blue Dream, Topaz T25, ANTG Rocky (optional)" },
];
